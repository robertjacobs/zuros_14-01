...
class FibaroZWaveHomeController(PollingProcessor):
	...
	def start(self):
		...
		rospy.loginfo("Started polling zwave sensors")
	...
	def stop(self):
		rospy.loginfo("Stopped polling zwave sensors")
	...
	def pollZWaveSensors(self):
		...
		try:
			...
			if id(self) + type(e) not in self._warned:
				rospy.logerror("Error while receiving data from ZWaveHomeController: %s" % e)
		...
		for device in data:
			...
			try:
				...
				rospy.loginfo("Added sensor with name %s" % device['name'])
				continue	
			...
			for sensor in self._sensorsAdded:
				if(str(sensor['id']) == str(communication_id)):
					if(str(sensor['properties']['value']) != str(device['properties']['value'])):
						...
						rospy.loginfo("value changed for device %s" % device['name'])
...
if __name__ == '__main__':
	...
	for sensorType in config.locations_config['ZAP']['sensors']:
		...
		if sensorType == 'ZWaveHomeController':
			sensor = FibaroZWaveHomeController(config.server_config['zwave_ip'])
